...
class KalmanDetectionFilter
{
  ...
  void addFilter(const samplereturn_msgs::NamedPoint& msg)
  {
    ...
    cv::setIdentity(KF->processNoiseCov, cv::Scalar(1e0));
    cv::setIdentity(KF->measurementNoiseCov, cv::Scalar(1e1));
  ...
  void drawFilterStates() {
    ...
    for (int i=0; i<filter_list_.size(); i++) {
      ...
      cv::Point mean(filter_list_[i]->statePost.at<float>(0),
          filter_list_[i]->statePost.at<float>(1));
      std::cout << "Drawn Point: " << mean << std::endl;
      float rad_x =  filter_list_[i]->errorCovPost.at<float>(0,0);
      float rad_y =  filter_list_[i]->errorCovPost.at<float>(1,1);
