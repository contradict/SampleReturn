...
void computeColorMask(const cv::Mat& color, cv::Mat& mask, int r_size, int g_size, int b_size, int win_size)
{
...
class Image_test
{
    ...
    Image_test::sources.push_back(depth_img);
      ...
      for (int i = 0; (i < (int)matches.size()) && (classes_visited < Image_test::num_classes); ++i)
      {
        ...
        if (visited.insert(m.class_id).second)
        {
          ...
          std::cout << "Template width: " << templates[m.template_id].width << std::endl;
          ...
          if (Image_test::learn_online)
          {
            ...
            subtractPlane(depth_img, depth_mask, chain, focal_length);
            computeColorMask(Image_test::color_img, color_mask, Image_test::r_size, Image_test::g_size, Image_test::b_size, Image_test::win_size);
  ...
  void colorCallback(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    //int from_to[] = {1,0};
    //cv::mixChannels(&lab_img,1,&lab_img,2,from_to,1);
    ////Image_test::color_img = lab_img;
    //Image_test::color_img = color_ptr->image;

    //cv::Mat mask;
    //computeColorMask(Image_test::color_img, mask, Image_test::l_size, Image_test::a_size, Image_test::b_size, Image_test::win_size);
